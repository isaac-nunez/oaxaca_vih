...
#save(capasits_oaxaca, file="capasits_oaxaca.RDa")
#save(cd4_oaxaca, file="cd4_oaxaca.RDa")
#save(cv_oaxaca, file="cv_oaxaca.RDa")
```
...
capasits_oaxaca <- group_by(capasits_oaxaca, id_paciente) %>% 
  slice(1)
...
nrow(filter(capasits_oaxaca, paciente_notificacion >="2018-01-01"))
nrow(filter(capasits_oaxaca_2018_2021, paciente_notificacion >="2018-01-01"))
```
...
capasits_oaxaca_t3 <- filter(capasits_oaxaca, paciente_notificacion>="2018-01-01") %>% 
  mutate(ano_diagnostico=year(paciente_notificacion)) 
...
cd4_oaxaca_t3 <- filter(cd4_oaxaca, id_paciente%in%capasits_oaxaca_t3$id_paciente)
...
cd4_oaxaca_base_toma_al_ingreso_t3 <- mutate(cd4_oaxaca_t3, 
  ...
  group_by(id_paciente) %>% 
  mutate(tipo_toma = "ingreso") %>% 
  left_join(select(capasits_oaxaca_t3, id_paciente,  paciente_ingreso, paciente_notificacion) %>% mutate(id_paciente=as.double(id_paciente)), by= "id_paciente") %>% 
  mutate(fecha_cd4_fecha_ingreso=ifelse(fecha_toma-paciente_ingreso>=0, fecha_toma-paciente_ingreso,
                                        -(fecha_toma-paciente_ingreso))) %>% 
  slice_min(fecha_cd4_fecha_ingreso)
...
cd4_tiempo_t3 <- function(x){
  c(str_c(median(filter(cd4_oaxaca_t3, ano_diagnostico=="2018"&!is.na(resultado)&tipo_toma==x)$resultado), " (", 
                         fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2018"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2018"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
                     
                     str_c(median(filter(cd4_oaxaca_t3, ano_diagnostico=="2019"&!is.na(resultado)&tipo_toma==x)$resultado), " (", fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2019"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2019"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
                     
                     str_c(median(filter(cd4_oaxaca_t3, ano_diagnostico=="2020"&!is.na(resultado)&tipo_toma==x)$resultado), " (", fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2020"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2020"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
                     
                     str_c(median(filter(cd4_oaxaca_t3, ano_diagnostico=="2021"&!is.na(resultado)&tipo_toma==x)$resultado), " (", fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2021"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cd4_oaxaca_t3, ano_diagnostico=="2021"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
...
###CARGA VIRAL
cv_oaxaca_t3 <- filter(cv_oaxaca, id_paciente%in%capasits_oaxaca_t3$id_paciente)
...
cv_tiempo_t3 <- function(x){
  c(str_c(median(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&!is.na(resultado)&tipo_toma==x)$resultado), " (", 
                         fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
                     
                     str_c(median(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&!is.na(resultado)&tipo_toma==x)$resultado), " (", fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
                     
                     str_c(median(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&!is.na(resultado)&tipo_toma==x)$resultado), " (", fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
                     
                     str_c(median(filter(cv_oaxaca_t3, ano_diagnostico=="2021"&!is.na(resultado)&tipo_toma==x)$resultado), " (", fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2021"&!is.na(resultado)&tipo_toma==x)$resultado)[2],
                           "-", fivenum(filter(cv_oaxaca_t3, ano_diagnostico=="2021"&!is.na(resultado)&tipo_toma==x)$resultado)[4], ")"),
...
cv_indetectable_t3 <- function(x, y){
  if(y==50){
  c(str_c(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&tipo_toma==x)$indetectable_50==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&tipo_toma==x)$indetectable_50==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&tipo_toma==x)), 3)*100, "%)"),
                          #  
                           str_c(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&tipo_toma==x)$indetectable_50==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&tipo_toma==x)$indetectable_50==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&tipo_toma==x)), 3)*100, "%)"),
                           #
                           str_c(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&tipo_toma==x)$indetectable_50==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&tipo_toma==x)$indetectable_50==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&tipo_toma==x)), 3)*100, "%)"),
                           #
                           str_c(sum(filter(cv_oaxaca_t3,ano_diagnostico=="2021"&tipo_toma==x)$indetectable_50==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2021"&tipo_toma==x)$indetectable_50==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2021"&tipo_toma==x)), 3)*100, "%)"),
                           #
                           str_c(sum(filter(cv_oaxaca_t3, tipo_toma==x)$indetectable_50==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, tipo_toma==x)$indetectable_50==T)/
                                         nrow(filter(cv_oaxaca_t3, tipo_toma==x)), 3)*100, "%)"))
  }else{
      c(str_c(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&tipo_toma==x)$indetectable_200==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&tipo_toma==x)$indetectable_200==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2018"&tipo_toma==x)), 3)*100, "%)"),
                          #  
                           str_c(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&tipo_toma==x)$indetectable_200==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&tipo_toma==x)$indetectable_200==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2019"&tipo_toma==x)), 3)*100, "%)"),
                           #
                           str_c(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&tipo_toma==x)$indetectable_200==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&tipo_toma==x)$indetectable_200==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2020"&tipo_toma==x)), 3)*100, "%)"),
                           #
                           str_c(sum(filter(cv_oaxaca_t3,ano_diagnostico=="2021"&tipo_toma==x)$indetectable_200==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, ano_diagnostico=="2021"&tipo_toma==x)$indetectable_200==T)/
                                         nrow(filter(cv_oaxaca_t3, ano_diagnostico=="2021"&tipo_toma==x)), 3)*100, "%)"),
                           #
                           str_c(sum(filter(cv_oaxaca_t3, tipo_toma==x)$indetectable_200==T), " (", 
                                 round(sum(filter(cv_oaxaca_t3, tipo_toma==x)$indetectable_200==T)/
                                         nrow(filter(cv_oaxaca_t3, tipo_toma==x)), 3)*100, "%)"))
  }
}
#Carga viral más reciente (sólo si al menos 6 meses de seguimiento)
